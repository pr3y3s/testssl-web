...
def csv_analysis(file, analysis_option):
    ...
    if analysis_option == "completo":
        ...
        cleanup_files()
        filtered_10 = filter_tls(ciphersuites_tls_10, 10)
        filtered_11 = filter_tls(ciphersuites_tls_11, 1)
        filtered_12 = filter_tls(ciphersuites_tls_12, 2)
        filtered_13 = filter_tls(ciphersuites_tls_13, 3)
...
def sql_extract(ciphersuites12, ciphersuites13, curves, analysis_option):
    """
    Extracts validated ciphersuites and curves and their recommendations from the database.
    ...
    if analysis_option == "stic221":
        ...
        cursor.execute("SELECT curve, ccn_stic_221_recommendation FROM curves")
        ...
        ccn_ciphers_13 = {row[0]: row[1] for row in cursor.fetchall()}
        cursor.execute("SELECT curve, ccn_stic_807_recommendation FROM curves")
        ccn_curves = {row[0]: row[1] for row in cursor.fetchall()}
...
def filter_tls(tuples_list, version_token):
    """
